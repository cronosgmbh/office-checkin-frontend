...
export class BookingsService {
   ...
   */
  getBookings(): Observable<{ bookings: CronosBooking[] }> {
    return this.api.get('/bookings');
  }

  remove(booking: CronosBooking) {
    return this.api.delete('/bookings/' + booking.id);
  }

  add(area: any, dates: Date[], startDate = '', endDate = '') {
    const parsedDates = [];
    dates.forEach(d => {
      parsedDates.push(moment(d).format('yyyy-MM-DD'));
    });
