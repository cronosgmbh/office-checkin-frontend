...
export class AreasService {
  ...
  constructor(
    ...
    private angularUser: AngularFireAuth,
    private api: ApiService) {}

  public getAllAreas(): Observable<{ areas: CronosArea[] }> {
    return this.api.get('/areas');
  }

  public getFullDatesForArea(area: string): Observable<{ dates: string[] }> {
    return this.api.get(`/areas/${area}/unavailable-dates`);
  }

  public getForecastForArea(area: string, daysInFuture = 7): Observable<Forecast> {
    return this.api.get(`/areas/${area}/forecast?days-in-future=${daysInFuture}`);
  }
